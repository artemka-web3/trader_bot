...
async def send_welcome(message: types.Message):
    ...
    if await is_in_pay_sys(message.from_user.id) and await check_if_subed(message.from_user.id):
        with open('intro.jpg', 'rb') as photo:
            await bot.send_photo(message.from_user.id, photo, """"Радар биржи" анализирует все минутные свечи акций торгуемых на Московской бирже. Если бот видит повышенный обьем в акции, то он сразу сигнализирует об этом.\n\nБот уведомляет:\n\n🔸 Какой обьем был куплен.\n🔸 Изменение цены на данном обьеме.\n🔸 Изменение цены за день.\n🔸 О количестве покупателей и продавцов на данном обьеме.\n\nЭтот инструмент должен быть у каждого инвестора""", reply_markup=keyb_for_subed)
    else:
        with open('intro.jpg', 'rb') as photo:
            await bot.send_photo(message.from_user.id, photo, """"Радар биржи" анализирует все минутные свечи акций торгуемых на Московской бирже. Если бот видит повышенный обьем в акции, то он сразу сигнализирует об этом.\n\nБот уведомляет:\n\n🔸 Какой обьем был куплен.\n🔸 Изменение цены на данном обьеме.\n🔸 Изменение цены за день.\n🔸 О количестве покупателей и продавцов на данном обьеме.\n\nЭтот инструмент должен быть у каждого инвестора""", reply_markup=keyb_for_unsubed)
